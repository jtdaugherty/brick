    ...
    where
        ui = C.center $ hLimit 80 $ vLimit 21 $
             (vBox [ pair
                   ...
                   , C.hCenter (str "Last clicked name:")
                   , str $ show $ _lastClickedName st
                   ])
        pair = hBox [ padRight (Pad 5) $
                      ...
                      (clickable Thing1 $ str "12345") `above`
...
appEvent (T.MouseDown (SBClick el n) _ _ _) = do
    ...
    case n of
        ...
        _ ->
            return ()
appEvent (T.MouseDown n _ _ _) =
    lastClickedName .= Just n
appEvent _ = return ()
...
main = do
    ...
    initialVty <- buildVty
    void $ M.customMain initialVty buildVty Nothing app (St Nothing Nothing)
