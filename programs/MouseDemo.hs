...
data Name = Info | Button1 | Button2 | Button3
          | Edited | EditedView
...
data St =
    St { _clicked :: [T.Extent Name]
       ...
       , _editText :: E.Editor String Name
       }
...
editLayer st =
  ...
  viewport EditedView Vertical $
  clickable Edited $
  E.renderEditor False (st^.editText)
...
appEvent st (T.VtyEvent (V.EvKey V.KUp [])) = M.vScrollBy (M.viewportScroll EditedView) (-1) >> M.continue st
appEvent st (T.VtyEvent (V.EvKey V.KDown [])) = M.vScrollBy (M.viewportScroll EditedView) 1 >> M.continue st
...
main = do
    ...
    void $ M.customMain buildVty Nothing app $ St [] Nothing
         $ E.editor Edited (str . unlines) (Just 25)
