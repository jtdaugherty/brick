...
data MenuEntryAction s n k =
    GenerateEvent k
    -- ^ The menu entry generates the specified event when activated
    | OpenSubmenu (Menu s n k)
    -- ^ The menu entry opens a submenu when activated
...
data MenuEntry s n k =
    MenuEntry { menuEntryTitle :: T.Text
              ...
              -- enabled
              , menuEntryAction :: MenuEntryAction s n k
              -- ^ The action to take when this entry is activated
              }
...
menuEntry title enabled ev = MIEntry $ MenuEntry title enabled $ GenerateEvent ev

submenu :: (s -> Bool) -> Menu s n k -> MenuItem s n k
submenu enabled m = MIEntry $ MenuEntry (menuTitle m) enabled $ OpenSubmenu m
...
renderMenu s m =
    ...
    where
        ...
        renderMenuEntry i e =
            ...
            vLimit 1 $
            padLeft (Pad 1) $
            case menuEntryAction e of
                GenerateEvent {} ->
                    padRight Max $ txt $ menuEntryTitle e
                OpenSubmenu sm ->
                    let label = (padRight Max $ txt $ menuEntryTitle e) <+> txt "->"
                    in if menuIsOpen sm
                       then (translateLayer (Location (menuWidth m, 0)) $ renderMenu s sm) `above` label
                       else label
            ...
            case is V.!? idx of
                Just (MIEntry entry) -> do
                    when (menuEntryEnabled entry s) $ do
                        -- Is the entry a submenu entry? If so,
                        -- activation opens the submenu. Otherwise it
                        -- activates the entry and closes the menu.
                        case menuEntryAction entry of
                            GenerateEvent e -> do
                                which %= closeMenu
                                handler e
                            OpenSubmenu sm ->
                                which.menuItemsL.ix idx .= MIEntry (entry { menuEntryAction = OpenSubmenu (openMenu sm) })
                _ -> return ()
